...
const char			HttpRequestDTO::kCRLF_[] = "\r\n";
const char			HttpRequestDTO::kWhitespace_[] = " \t";
const std::size_t	HttpRequestDTO::kPortMax_ = 65535;

HttpRequestDTO::HttpRequestDTO(const std::string &raw_request)
	: port_(80), content_length_(0),
	parse_state_(kParseRequestLine), state_(RequestState::kPartial) {
		ParseRawString_(raw_request);
}

HttpRequestDTO::~HttpRequestDTO() {}

std::size_t
HttpRequestDTO::ParseRawString_(const std::string &raw_request) {
	offset_ = 0;
...
std::string	HttpRequestDTO::GetMethod() const {
	return method_;
}

std::string	HttpRequestDTO::GetRequestTarget() const {
	return request_target_;
}

std::string	HttpRequestDTO::GetPath() const {
	return path_;
}

HttpRequestDTO::QueriesMap	HttpRequestDTO::GetQueries() const {
	return queries_;
}

std::string	HttpRequestDTO::GetQueryValue(const std::string &query_name) const {
...
std::string HttpRequestDTO::GetQueryString() const {
	if (queries_.empty()) {
		return "";
	}
...
bool	HttpRequestDTO::HasQuery(const std::string &query_name) const {
	return queries_.count(query_name) > 0;
}

std::string	HttpRequestDTO::GetHttpVersion() const {
	return http_version_;
}

HttpRequestDTO::HeadersMap	HttpRequestDTO::GetHeaders() const {
	return headers_;
}
...
std::string	HttpRequestDTO::GetHost() const {
	return host_;
}

std::size_t	HttpRequestDTO::GetPort() const {
	return port_;
}

std::string HttpRequestDTO::GetBody() const {
	return body_;
}

bool	HttpRequestDTO::HasHeader(const std::string &header_name) const {
	return headers_.count(ToLowerString(header_name)) > 0;
}

void	HttpRequestDTO::ParseRequestLine_(const std::string &raw_request) {
...
void	HttpRequestDTO::ParseMethod_(const std::string &raw_request) {
...
void	HttpRequestDTO::ParseRequestTarget_(const std::string &raw_request) {
...
void	HttpRequestDTO::ParseQueryString_(const std::string &query_string) {
...
void	HttpRequestDTO::AddQuery_(
					const std::string &name, const std::string &value) {
	queries_.insert(make_pair(name, value));
}

bool	HttpRequestDTO::IsValidPath_(const std::string &path) const {
	return !path.empty() && path[0] == '/';
}

void	HttpRequestDTO::ParseHttpVersion_(const std::string &raw_request) {
...
void	HttpRequestDTO::ParseHeaders_(const std::string &raw_request) {
...
bool	HttpRequestDTO::ParseHeader_(const std::string &header) {
...
std::string HttpRequestDTO::ParseHeaderName_(const std::string &header) {
...
std::string HttpRequestDTO::ParseHeaderValue_(const std::string &header) {
...
void	HttpRequestDTO::AddHeader_(
		const std::string &name, const std::string &value) {
	headers_.insert(std::make_pair(name, value));
}

// https://datatracker.ietf.org/doc/html/rfc7230#section-3.2.6
bool	HttpRequestDTO::IsValidHeaderName_(const std::string &name) const {
...
bool	HttpRequestDTO::IsValidHeaderValue_(const std::string &value) const {
	return !value.empty() && ContainOnlyVisibleChars_(value);
}

bool	HttpRequestDTO::ContainOnlyVisibleChars_(const std::string &str) const {
...
// Host = uri-host [ ":" port ]
void	HttpRequestDTO::ParseHost_() {
...
void	HttpRequestDTO::ParsePort_(const std::string &port_str) {
...
void	HttpRequestDTO::ParseContentLength_() {
	const std::string valid_chars = "0123456789";
...
void	HttpRequestDTO::ParseBody_(const std::string &raw_request) {
...
RequestState::State	HttpRequestDTO::GetState() const {
	return state_;
}
